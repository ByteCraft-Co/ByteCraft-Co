# ByteCraft

Software Engineer · Systems Builder · Open Source

I build practical software with a focus on correctness, clarity, and long-term maintainability.  
Most of my work centers on developer tooling, backend systems, and low-level understanding of how software runs.

## Now

- Building and maintaining open source libraries and tooling
- Designing systems with clear architecture and predictable behavior
- Focusing on reliability, documentation, and long-term support

## Tech

### Languages
![Assembly](https://img.shields.io/badge/Assembly-6E4C13?style=flat)
![TypeScript](https://img.shields.io/badge/TypeScript-3178C6?style=flat&logo=typescript&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=flat&logo=javascript&logoColor=black)
![Python](https://img.shields.io/badge/Python-3776AB?style=flat&logo=python&logoColor=white)
![Java](https://img.shields.io/badge/Java-007396?style=flat)
![C#](https://img.shields.io/badge/C%23-512BD4?style=flat&logo=csharp)
![C++](https://img.shields.io/badge/C++-00599C?style=flat&logo=cplusplus)
![C](https://img.shields.io/badge/C-A8B9CC?style=flat&logo=c)
![Rust](https://img.shields.io/badge/Rust-000000?style=flat&logo=rust)
![Ruby](https://img.shields.io/badge/Ruby-CC342D?style=flat&logo=ruby)
![Kotlin](https://img.shields.io/badge/Kotlin-7F52FF?style=flat&logo=kotlin)
![Dart](https://img.shields.io/badge/Dart-0175C2?style=flat&logo=dart)

### Frontend
![React](https://img.shields.io/badge/React-20232A?style=flat&logo=react)
![HTML](https://img.shields.io/badge/HTML5-E34F26?style=flat&logo=html5&logoColor=white)
![CSS](https://img.shields.io/badge/CSS3-1572B6?style=flat&logo=css3)
![TSX](https://img.shields.io/badge/TSX-3178C6?style=flat)

### Backend & Systems
![Node.js](https://img.shields.io/badge/Node.js-339933?style=flat&logo=node.js&logoColor=white)
![.NET](https://img.shields.io/badge/.NET-512BD4?style=flat&logo=dotnet)
![Arduino](https://img.shields.io/badge/Arduino-00979D?style=flat&logo=arduino)

### Tooling
![Git](https://img.shields.io/badge/Git-F05032?style=flat&logo=git&logoColor=white)

## Projects

| Repository | Description |
|-----------|-------------|
| **ledgerkit-core** | Core libraries and foundational tooling for the LedgerKit project |
| **INVDB** | Inventory database system focusing on structure and data handling |
| **code-evolution** | Experiments and studies on code growth, refactoring, and evolution |
| **CyberSec-1** | Cybersecurity-related exercises and foundational security concepts |

## Philosophy

I prefer boring solutions that work.  
I value readable code over clever code.  
I ship, iterate, and document.
